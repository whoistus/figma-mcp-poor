...
import type { SerializedFlowData, FlowGraph } from "./types.js";
...
export function registerTools(server: McpServer, bridge: BridgeLike): void {
  ...
  server.tool(
    ...
    "Selected nodes with CSS properties. Text always included. Options: flatten_text, include_screenshot.",
    {
      depth: z
        ...
        .max(5)
        .optional()
        .describe("Depth (default 1, max 5)"),
      flatten_text: z
        .boolean()
        .optional()
        .describe("Flat list of all text in subtree"),
      include_screenshot: z
        .boolean()
        .optional()
        .describe("Inline PNG thumbnail"),
      max_width: z
        .number()
        .optional()
        .describe("Screenshot max width px (default 800)"),
    },
  ...
  server.tool(
    ...
    "Node details by ID with CSS properties. Supports pagination for large children lists via childrenOffset/childrenLimit.",
    {
      ...
      properties: z
        ...
        .optional()
        .describe("Filter categories (default: all)"),
      flatten_text: z
        .boolean()
        .optional()
        .describe("Flat list of all text in subtree (capped at 500)"),
      include_screenshot: z
        .boolean()
        .optional()
        .describe("Inline PNG thumbnail"),
      ...
      childrenOffset: z
        ...
        .min(0)
        .optional()
        .describe("Skip first N children (for pagination)"),
      childrenLimit: z
        .number()
        .min(1)
        .max(500)
  ...
  server.tool(
    ...
    async ({ styleType }) => {
      const data = (await bridge.request("get_styles", {
        styleType,
      })) as SerializedStyle[];
      const formatted = data.map(formatStyle);
      return {
        content: [{ type: "text", text: J(truncateResponse(formatted)) }],
      };
    },
  );
  ...
  server.tool(
    ...
    async ({ collection, type }) => {
      const data = (await bridge.request("get_variables", {
        collection,
        type,
      })) as SerializedVariable[];
  ...
  server.tool(
    ...
    async ({ name }) => {
      const data = (await bridge.request("get_components", {
        name,
  ...
  server.tool(
    ...
    "Design brief for a node. Brief=markdown, detailed=JSON.",
    {
      ...
      format: z
        .enum(["brief", "detailed"])
        .optional()
        .describe("brief (default) or detailed"),
    },
    async ({ nodeId, format: formatArg }) => {
      ...
      const formatted = {
        node: formatNode(data.node),
        children: data.children.map(formatNode),
      };
      return {
        content: [{ type: "text", text: J(truncateResponse(formatted)) }],
      };
  ...
  server.tool(
    ...
    async ({ nodeId, scale: scaleArg, format: formatArg, max_width }) => {
      ...
      const data = (await bridge.request(
        "get_screenshot",
        { nodeId, scale, format, max_width: max_width ?? 800 },
        60_000,
      )) as {
        base64: string;
        mimeType: string;
  ...
  server.tool(
    ...
    "Complete design summary in one call: structure, all text, colors, components, screenshot. Use this first.",
    {
      ...
      include_screenshot: z
        .boolean()
        .optional()
        .describe("Include PNG (default true)"),
      max_width: z.number().optional().describe("Max width px (default 800)"),
      depth: z
        .number()
        .min(0)
        .max(5)
        .optional()
        .describe("Structure tree depth (default 2, max 5)"),
    },
    async ({ nodeId, include_screenshot, max_width, depth }) => {
      ...
      const textLines = data.texts.map((t) => {
        const font =
          t.fontName && t.fontName !== "mixed" ? t.fontName.family : "";
        const size =
          t.fontSize && t.fontSize !== "mixed" ? `${t.fontSize}` : "";
      ...
      const compLines = [...byComponent.entries()].map(
        ([componentName, entry]) => {
          const countSuffix =
            entry.instances.size > 1 ? ` x${entry.instances.size}` : "";
          const uniqueTexts = [...new Set(entry.texts)]
            .map((t) => `"${t}"`)
            .join(", ");
          return uniqueTexts
            ? `${componentName}${countSuffix}: ${uniqueTexts}`
            : `${componentName}${countSuffix}`;
        },
      );
  ...
  server.tool(
    ...
    "Prototype flows and interactions. Shows starting points and all click/hover/press connections between frames. Useful for understanding navigation logic.",
    {
      nodeId: z
        .string()
        .optional()
        .describe("Scope to a subtree (default: entire current page)"),
      limit: z
        .number()
        .min(1)
        .max(1000)
        .optional()
        .describe("Max connections to return (default 200)"),
    },
    async ({ nodeId, limit }) => {
      const data = (await bridge.request("get_flows", {
        nodeId,
      })) as SerializedFlowData;
  ...
  server.tool(
    ...
    "Structured flow graph: real prototype connections or explicit no-flow signal. Returns FlowGraph JSON (nodes, edges, paths) for deterministic AI navigation analysis.",
    {
      nodeId: z
        .string()
        .optional()
        .describe("Scope to subtree (default: entire current page)"),
      maxDepth: z
        .number()
        .min(1)
        .max(20)
        .optional()
        .describe("BFS path depth limit (default 10)"),
      includeInference: z
        .boolean()
        .optional()
        .describe("Generate inferred edges when no real flow exists (default false)"),
    },
    async ({ nodeId, maxDepth, includeInference }) => {
      const data = (await bridge.request(
        "get_user_flow_context",
        { nodeId, maxDepth, includeInference },
        60_000,
      )) as FlowGraph;
      return { content: [{ type: "text", text: J(data) }] };
    },
...
function formatFlatTexts(
  flatTexts: unknown[],
): Array<{ t: string; f?: string; c?: string }> {
  return flatTexts.map((item: unknown) => {
    const t = item as Record<string, unknown>;
    const fills = t.fills as SerializedPaint[] | undefined;
    const result: { t: string; f?: string; c?: string } = {
      t: String(t.characters || ""),
    };
...
function buildSingleNodeResponse(data: Record<string, unknown>) {
  ...
  if (
    ...
    (screenshot as Record<string, unknown>).base64
  ) {
    const ss = screenshot as { base64: string; mimeType: string };
    content.push({
      type: "image",
      data: ss.base64,
      mimeType: ss.mimeType || "image/png",
    });
  }
